...
public class DataExportHelper(ElectronicObserverContext db, ToolService toolService)
{
					...
					void ProcessData(IFleetData fleet, IFleetData attackerFleet, FleetFlag fleetFlag, int indexOffset)
					{
						foreach ((IShipData? ship, int index) in fleet.MembersWithoutEscaped!.Select((s, i) => (s, i)))
						{
							...
							airBattleData.Add(new()
							{
								...
								Stage1 = new()
								{
									PlayerAircraftTotal = airAttackUnit.AircraftTotalStage1Friend,
									PlayerAircraftLost = airAttackUnit.AircraftLostStage1Friend,
									EnemyAircraftTotal = airAttackUnit.AircraftTotalStage1Enemy,
									EnemyAircraftLost = airAttackUnit.AircraftLostStage1Enemy,
								},
								Stage2 = new()
								{
									PlayerAircraftTotal = airAttackUnit.AircraftTotalStage2Friend,
									PlayerAircraftLost = airAttackUnit.AircraftLostStage2Friend,
									EnemyAircraftTotal = airAttackUnit.AircraftTotalStage2Enemy,
									EnemyAircraftLost = airAttackUnit.AircraftLostStage2Enemy,
								},
		...
		foreach (SortieNode node in sortieDetail.Nodes.Where(n => exportFilter?.MatchesFilter(n) ?? true))
		{
			...
			foreach (PhaseBaseAirRaid airBaseRaid in phases.OfType<PhaseBaseAirRaid>())
			{
				...
				airBattleData.Add(new()
				{
					...
					PlayerAircraft = airBaseRaid.AircraftTotalStage1Friend,
					PlayerAircraftLost = airBaseRaid.AircraftLostStage1Friend,
					EnemyAircraft = airBaseRaid.AircraftTotalStage1Enemy,
					EnemyAircraftLost = airBaseRaid.AircraftLostStage1Enemy,
	...
	private static AirBattleExportModel MakeAirBattleExport(BattleNode node,
		...
		=> new()
		{
			...
			Stage1 = new()
			{
				PlayerAircraftTotal = airBattle.AircraftTotalStage1Friend,
				PlayerAircraftLost = airBattle.AircraftLostStage1Friend,
				EnemyAircraftTotal = airBattle.AircraftTotalStage1Enemy,
				EnemyAircraftLost = airBattle.AircraftLostStage1Enemy,
			},
			Stage2 = new()
			{
				PlayerAircraftTotal = airBattle.AircraftTotalStage2Friend,
				PlayerAircraftLost = airBattle.AircraftLostStage2Friend,
				EnemyAircraftTotal = airBattle.AircraftTotalStage2Enemy,
				EnemyAircraftLost = airBattle.AircraftLostStage2Enemy,
			},
			AntiAirCutIn = new()
			{
	...
	private static CommonDataExportModel MakeCommonData(BattleNode node, bool isFirstNode,
		...
		PhaseSearching searching) => new()
		{
			...
			Rank = WinRank(node.BattleResult?.Rank),
